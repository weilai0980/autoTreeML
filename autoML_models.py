...
def gbt_n_estimatior(maxnum, 
    ...
    for i in range(10,maxnum + 1,10):
        ...
        clf.fit(x_tr, tmpy)
        py_val = clf.predict(x_val)
        
        if bool_clf == False:
            
            tmp_val_err = rmse(y = y_val, 
                               yhat = py_val) 
            #sqrt(sum((py_val - y_val)*(py_val - y_val))/len(y_val))
            
            n_tree_val_err.append((i, tmp_val_err))
    ...
    return min(n_tree_val_err, key = lambda x: x[1]) if bool_clf == False else max(n_tree_val_err, key = lambda x: x[1]),\
           best_model,\
           rmse(y = tmpy,
                yhat = best_model.predict(x_tr))
           #utils_evaluation_score(X, Y, bool_clf, best_model) 
...
def gbt_tree_para(x_tr, 
    ...
    for i in depth_range:
        ...
        if bool_clf == False:
            ...
            clf = GradientBoostingRegressor(n_estimators = fix_n_est, 
                                            learning_rate = fix_lr,
                                            max_depth = i,
                                            max_features ='sqrt')
        else:
            clf = GradientBoostingClassifier(n_estimators = fix_n_est,
                                             learning_rate = fix_lr,
                                             max_depth = i,
                                             max_features ='sqrt')
        ...
        if bool_clf == False:
            ...
            depth_val_err.append((i, tmp_val_err))
        ...
        # classification
            ...
            tmp_val_err = clf.score(x_val, y_val)
            depth_val_err.append((i, tmp_val_err))
    ...
    # the best pair of hyper-para and validataion errors, best model, training errors  
    return min(depth_val_err, key = lambda x: x[1]) if bool_clf == False else max(depth_val_err, key = lambda x: x[1]),\
           best_model,\
           rmse(y = tmpy,
                yhat = best_model.predict(x_tr))
...
def gbt_train_validate(x_tr, 
    ...
    n_err, model0, train_err0 = gbt_n_estimatior(200, 
                                                 x_tr, 
                                                 y_tr, 
                                                 x_val, 
                                                 y_val, 
                                                 fix_lr, 
                                                 bool_clf)
    print("n_estimator, training and validation errors: ", train_err0, n_err)
    ...
    depth_err, model1, train_err1 = gbt_tree_para(x_tr, 
                                                  ...
                                                  n_err[0], 
                                                  bool_clf)
    print("depth, training and validation errors: ", train_err1, depth_err)
    ...
    np.savetxt(path_pred + "pytrain_gbt.txt", zip(y_tr, py), delimiter=',')
    
    # save testing prediction under the best model
    if len(x_test) != 0:
        
        py = best_model.predict(x_test)
        np.savetxt(path_pred + "pytest_gbt.txt", zip(y_test.reshape((len(y_test),)), py), delimiter=',')
